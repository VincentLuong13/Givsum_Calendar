...
def get_calendar_variables(c_year = 0, c_month = 0, c_week = 0):
    ...
    if c_year != 0:
        today_date = datetime.datetime.today().replace(year = c_year, day = 1)
    if c_month !=0:
        today_date = datetime.datetime.today().replace(month = c_month,day = 1)
    if c_week !=0:
        today_date = datetime.datetime.today().replace(c_week = c_week,day = 1)
