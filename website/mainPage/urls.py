...
urlpatterns = [
    ...
    re_path(r'^(?P<view>month|week|year)\/(?P<year>[1-9]?[0-9]{3})-(?P<month>[0-1]?[0-9])-(?P<day>[0-3]?[0-9])\/filters\=(?P<filter>all|myevents|friends|ongoing)\/$', views.cal, name = 'cal'), #month/2018-10-3, changes the current view using the desired view type, and the date. Uses calendarpage.html to create the calendar
